...
int main(int argc, char *argv[])
{
  ...
  if (header_end != NULL) {
    ...
    // Find the first space in the header (start of the status code)
    char *status_code_start = strchr(response, ' ');

    if (status_code_start != NULL) {
      // Move to the next character after the space to get the status code
      int status_code;
      if (sscanf(status_code_start + 1, "%d", &status_code) == 1) {
        printf("[%s] : [%d]\n",address, status_code);
      } else {
        fprintf(stderr, "Invalid status code format\n");
      }
    } else {
